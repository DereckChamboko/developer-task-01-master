...
public class CreditsServiceImplTest {
    ...
    public void setUp() throws Exception {
        ...
        when(requestDao.save(any(SubscriberRequest.class))).then(Base.SUBSCRIBER_REQUEST);
        when(platform.enquireBalance(anyString(), anyString())).then(Base.SUCCESSFUL_BALANCE_ENQUIRY);
        when(platform.creditSubscriberAccount(any(INCreditRequest.class))).then(Base.SUCCESSFUL_CREDIT_REQUEST);
